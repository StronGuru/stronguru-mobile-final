...
export const useChatNotifications = () => {
  ...
  // Gestione delle notifiche ricevute quando l'app è in foreground
  useEffect(() => {
    const subscription = Notifications.addNotificationReceivedListener(notification => {
      const data = notification.request.content.data;
      
      // Se è una notifica chat
      if (data?.type === 'chat') {
        console.log('📱 [UseChatNotifications] Notifica chat ricevuta:', {
          roomId: data.roomId,
          messageId: data.messageId,
          senderId: data.senderId
        });

        handleChatNotificationReceived(data);
      }
    });
    ...
    const responseSubscription = Notifications.addNotificationResponseReceivedListener(response => {
      ...
      // Se l'utente ha tappato su una notifica chat
      if (data?.type === 'chat') {
        console.log('📱 [UseChatNotifications] Utente ha tappato notifica chat:', {
          roomId: data.roomId,
          messageId: data.messageId
        });
  ...
  }, [handleChatNotificationReceived, handleChatNotificationTap]);
